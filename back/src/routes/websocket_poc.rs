...
pub async fn ws_join<'a>(
    ...
    socket.channel(move |stream| {
        Box::pin(async move {
            ...
            println!("Before try. User id: {user_id}");
            while tries < MAX_RETRIES {
                println!("Retry number: {tries} for user id: {user_id}");
                if let Ok(mut user_map_guard) = user_map.try_lock() {
                    ...
                    while user_map_guard.contains_key(&user_id) {
                        ...
                        println!("User ID {user_id} already exists, generating a new one.");
                        user_id = random::get_inc(u128::MIN, u128::MAX);
                        println!("New user ID generated: {user_id}");
                    }
                    user_map_guard.insert(user_id, arc_sender.clone());
                    println!("User map updated with user id: {user_id}");
                    break;
                } else {
            ...
            if tries == MAX_RETRIES {
                ...
                println!("Failed to lock the user map after {MAX_RETRIES} tries. Returning error.");
                ...
                match message {
                    ...
                    Ok(Message::Text(text_message)) => {
                        ...
                        let client_message: ClientMessage = match serde_json::from_str(&text_message) {
                            Ok(msg) => msg,
                            Err(e) => {
                                println!("Failed to parse message from user {user_id}: {e}");
                                continue; // Skip this message
                            }
                        };
                        println!("Received message from user {user_id}: {client_message:?}");
                        match &client_message {
                            ClientMessage::BoardUpdate(board_update) => {
                                // Handle the board update message
                                println!("User {user_id} sent a board update: {board_update:?}");
                            ...
                            ClientMessage::LinesDestroyed(line_shapes) => {
                                ...
                                println!("User {user_id} sent a lines destroyed message: {line_shapes:?}");
                                ...
                                if keys.len() < 2 {
                                    println!("Not enough users to send lines destroyed message to. Skipping.");
                                ...
                                println!("User {user_id} sent a lines destroyed message to user {victim_id}: {line_shapes:?}");
                            ...
                            ClientMessage::GameOver => {
                                ...
                                println!("User {user_id} sent a game over message.");
                    ...
                    Ok(m) => {
                        println!("Unhandled message type: {m:?}");
                    }
                    Err(_) => {
    ...
    // } else {
    //     panic!("Expected a BoardUpdate message, got: {board_message}");
    // }
...
pub async fn send_lines_destroyed_message (victim_id: u128, client_message: &ClientMessage, user_map: &State<UserMap>) {
    ...
    println!("Sending lines destroyed message to user {victim_id}: {:?}", server_message);
    if let Some(ws) = map_lock.get(&victim_id) {
        ...
        println!("User {victim_id} not found in user map. Not sending message.");
    }
}
pub async fn broadcast_message(source_user_id: u128, client_message: &ClientMessage, user_map: &State<UserMap>) {
    ...
    println!("Broadcasting board update from user {source_user_id}: {:?}", server_message);
...
pub async fn ws_broadcast(content: &str, user_map: &State<UserMap>) -> crate::response::Response {
    ...
    println!("New broadcast request received.");
